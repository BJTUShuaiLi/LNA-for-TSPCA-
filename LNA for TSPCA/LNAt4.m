function [xopt,Out] = LNAt4(n, s, data,  probname, pars)

% This code aims at solving the tensor sparse PCA with form
%
%          min_{x\in R^n} -\cal{A}x^4  s.t. x^{T}x=1, \|x\|_0<=s
%
...
%     data    : 
%   
%               data.A  --  4 order n dims hyper symmetric tensor (required)
...
switch      probname
    case 'TSPCA'; [xopt,iter] = LNAt4_TSPCA(x,y,data);
%     otherwise  ; 
end
